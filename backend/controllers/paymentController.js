...
exports.dummyCheckout = async (req, res) => {
  try {
    ...
    let coupon = null;
    if (couponId && String(couponId).length === 24) {
    ...
    // Create transaction record (works for both DB coupons and local JSON coupons)
    const transaction = await Transaction.create({
      user_id: req.userId,
      coupon_id: couponId,
      payment_id: `TXN_${Date.now()}_${Math.floor(Math.random() * 1000)}`,
      amount: amount || (coupon ? coupon.price : 0),
